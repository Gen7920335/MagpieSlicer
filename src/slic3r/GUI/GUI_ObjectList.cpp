...
public:
    ...
    {   // ORCA draw selection background to improve consistency between platforms
        dc.SetBrush(StateColor::darkModeColorFor(wxColour("#D7E9FD")));
    ...
    {   // ORCA draw focus rectangle to improve consistency between platforms
        dc.SetPen(  StateColor::darkModeColorFor(wxColour("#398FF4")));
        dc.DrawRectangle(rect);
    }
...
{
    // Plater already remapped all model configurations. This pass only refreshes rows;
    // filtering/collapsing the object tree must never decide which settings get updated.
    for (size_t i = 0; i < m_objects->size(); ++i) {
        const ModelObject *object = (*m_objects)[i];
        const int object_filament = object->config.has("extruder") ? object->config.extruder() : 1;
        const wxDataViewItem object_item = m_objects_model->GetItemById(i);
        if (object_item)
            m_objects_model->SetExtruder(wxString::Format("%d", object_filament), object_item);
        for (size_t v = 0; v < object->volumes.size(); ++v) {
            const wxDataViewItem item = m_objects_model->GetItemByVolumeId(i, v);
            if (item) {
                const auto &config = object->volumes[v]->config;
                m_objects_model->SetExtruder(wxString::Format("%d",
                    config.has("extruder") ? config.extruder() : object_filament), item);
            }
        }
        for (const auto &range : object->layer_config_ranges) {
            const wxDataViewItem item = m_objects_model->GetItemByLayerRange(i, range.first);
            if (item)
                m_objects_model->SetExtruder(wxString::Format("%d",
                    range.second.has("extruder") ? range.second.extruder() : 0), item);
        }
    }
}
