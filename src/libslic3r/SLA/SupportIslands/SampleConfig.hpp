...
// Debug-only SVG storage from PrusaSlicer is intentionally disabled in Magpie.
// It is unrelated to support generation and would write files while slicing.
...
struct PrepareSupportConfig
{
    ...
    float peninsula_min_width = static_cast<float>(scale_(2)); // [in scaled mm]

    // Distance from previous layer part to still supported
    float peninsula_self_supported_width = static_cast<float>(scale_(1.5)); // [in scaled mm]
    ...
    float removing_delta = static_cast<float>(scale_(5.));
