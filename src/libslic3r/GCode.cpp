...
static bool lesic_is_small_label_plate(const Calib_Params &params)
{
    return params.lesic_small_bed ||
        (params.lesic_circle_diameter > 0.0 && params.lesic_circle_diameter < 180.0);
}

static double lesic_bottom_label_stroke_width(const Calib_Params &params)
{
    return lesic_is_small_label_plate(params) ?
...
static std::vector<double> lesic_ring_label_values(double mvs_min, double mvs_max)
{
    ...
    if (values.empty() || std::abs(values.back() - mvs_max) > 1e-9)
        values.push_back(mvs_max);
    return values;
}
...
static double lesic_bottom_label_height(const Calib_Params &params, const std::vector<std::string> &lines)
{
    ...
    return std::min(
        std::max(1.0, std::min(h_by_width, h_by_height)),
        lesic_is_small_label_plate(params) ? 4.0 : 10.0);
}
...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        const std::string gcode_generator = std::string(GCODE_COMPATIBLE_PRODUCER) + " " SLIC3R_DISPLAY_VERSION;
        file.write_format("; generated by %s on %s\n", gcode_generator.c_str(), Slic3r::Utils::local_timestamp().c_str());
        if (is_bbl_printers)
    ...
    const bool                               prime_extruder)
{
    ...
    switch (print.calib_mode()) {
        ...
        case CalibMode::Calib_LESIC: {
            ...
            const int temp_band = std::max(0, m_layer_index) / layers_per_temp;
            const double temp = lesic_temperature_for_band(params.start, params.end, params.step, temp_band);
...
std::string GCode::extrude_support(const ExtrusionEntityCollection &support_fills, const ExtrusionRole support_extrusion_role)
{
    ...
    ++m_support_extrusion_context_depth;
    ScopeGuard support_context_guard([this]() { --m_support_extrusion_context_depth; });
    ...
    if (!support_fills.entities.empty()) {
        ...
        const bool low_temperature_controls_interface =
            m_config.single_nozzle_low_temperature_interface.value &&
            m_config.support_interface_filament.value == 0;
        const auto normal_temperature_for_current_extruder = [this]() {
            ...
            const ConfigOptionInts &temperatures = this->on_first_layer() ?
                m_config.nozzle_temperature_initial_layer : m_config.nozzle_temperature;
            return temperatures.values.empty() ? 0 : temperatures.get_at(extruder_id);
        };
        ...
        for (const ExtrusionEntity *ee : extrusions) {
            ...
            if (!low_temperature_controls_interface) {
...
{
    if (!m_config.support_interface_temperature_drop_tower.value ||
        !m_config.single_nozzle_low_temperature_interface.value ||
        m_config.support_interface_filament.value != 0 ||
    ...
    return std::any_of(
               m_config.nozzle_temperature.values.begin(),
               m_config.nozzle_temperature.values.end(),
               [interface_temperature](int temperature) { return temperature > interface_temperature; }) ||
           std::any_of(
               m_config.nozzle_temperature_initial_layer.values.begin(),
               m_config.nozzle_temperature_initial_layer.values.end(),
               [interface_temperature](int temperature) { return temperature > interface_temperature; });
}
    ...
    const ExtrusionPath &path, bool cooling_transition)
{
    ...
    if (path_length_mm > slow_tail_length_mm + EPSILON) {
        ...
        Polyline3 slow_polyline;
        const bool split = path.polyline.split_at_length(
            scale_(path_length_mm - slow_tail_length_mm), &fast_polyline, &slow_polyline);
        if (split && fast_polyline.points.size() >= 2 && slow_polyline.points.size() >= 2) {
            gcode += "; temperature drop tower cooling fast section\n";
            gcode += this->_extrude(
                ExtrusionPath(std::move(fast_polyline), path), "temperature drop tower", 30.0);
            gcode += "; temperature drop tower cooling final 10 mm\n";
            gcode += this->_extrude(
                ExtrusionPath(std::move(slow_polyline), path), "temperature drop tower", 10.0);
        } else {
            gcode += "; temperature drop tower cooling final 10 mm\n";
            gcode += this->_extrude(path, "temperature drop tower", 10.0);
        }
    } else {
        gcode += "; temperature drop tower cooling final 10 mm\n";
        gcode += this->_extrude(path, "temperature drop tower", 10.0);
    }
    gcode += "; temperature drop tower cooling pass end\n";
    m_temperature_drop_tower_last_print_z = m_nominal_z;
    return gcode;
}

int GCode::low_temperature_support_interface_temperature(ExtrusionRole role) const
{
    if (role == erSupportMaterialInterfaceSublayer &&
        m_config.support_interface_sublayer_temperature.value > 0)
        return m_config.support_interface_sublayer_temperature.value;
    return m_config.support_interface_temperature.value;
}
...
std::string GCode::start_low_temperature_support_interface(ExtrusionRole role)
{
    ...
    const unsigned int filament_id = m_writer.filament()->id();
    const ConfigOptionInts &normal_temperatures = on_first_layer()
        ? m_config.nozzle_temperature_initial_layer : m_config.nozzle_temperature;
    if (normal_temperatures.values.empty())
        return {};
    const int normal_temperature = normal_temperatures.get_at(filament_id);
    const int interface_temperature = low_temperature_support_interface_temperature(role);
    ...
    if (temperature_changes) {
        ...
        const bool use_temperature_drop_tower =
            ...
            !tower_already_printed &&
            this->build_temperature_drop_tower_path(temperature_drop_tower_path);
        ...
        const bool use_auxiliary_fan =
            ...
            !m_config.additional_cooling_fan_speed.values.empty() &&
            normal_temperature > interface_temperature &&
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed, ExtrusionToolHint tool_hint)
{
    ...
    const bool low_temperature_interface_role =
        ...
        (path.role() == erIroning && m_support_extrusion_context_depth > 0);
    ...
    else if (use_low_temperature_interface && low_temperature_interface_role &&
             m_low_temperature_support_interface_active && m_writer.filament() != nullptr) {
        const int target_temperature = low_temperature_support_interface_temperature(path.role());
        if (target_temperature > 0 &&
            target_temperature != m_low_temperature_support_interface_target_temperature) {
            gcode += m_writer.set_temperature(target_temperature, false, m_writer.filament()->id());
