...
{
    if (!enabled || interface_total <= 1 || interface_number < 2)
        return false;

    const int first = std::clamp(start_layer, 2, interface_total);
    const int last  = std::clamp(end_layer, first, interface_total);
...
{
    SupportProfileStage timing("support-interface", "Stabilize interface footprints", intermediate_layers.size());
    // Interface material may replace base material inside an existing support
    // island, but it must never create printable area outside that island. The
    // old tolerance outset widened the interface target beyond the actual body,
    // leaving edge strips with no support underneath.
    (void) support_tolerance;
    const size_t layer_count = intermediate_layers.size();
    assert(interface_targets.size() == layer_count);
    assert(base_interface_targets.size() == layer_count);
    assert(interface_layers.size() == layer_count);
    assert(base_interface_layers.empty() || base_interface_layers.size() == layer_count);

    auto allocate_like = [&layer_storage](SupportGeneratorLayer &source, SupporLayerType type) {
        SupportGeneratorLayer &layer = layer_storage.allocate(type);
        layer.print_z  = source.print_z;
        layer.bottom_z = source.bottom_z;
        layer.height   = source.height;
        layer.bridging = source.bridging;
        return &layer;
    };

    for (size_t layer_idx = 0; layer_idx < layer_count; ++layer_idx) {
        if (interface_targets[layer_idx].empty() && base_interface_targets[layer_idx].empty())
            continue;

        SupportGeneratorLayer &intermediate = *intermediate_layers[layer_idx];
        SupportGeneratorLayer *interface = interface_layers[layer_idx];
        SupportGeneratorLayer *base_interface =
            base_interface_layers.empty() ? nullptr : base_interface_layers[layer_idx];

        Polygons printable_area = intermediate.polygons;
        if (interface != nullptr)
            append(printable_area, interface->polygons);
        if (base_interface != nullptr)
            append(printable_area, base_interface->polygons);
        if (printable_area.empty())
            continue;

        printable_area = union_(std::move(printable_area));

        Polygons stable_interface;
        if (!interface_targets[layer_idx].empty())
            stable_interface = intersection(interface_targets[layer_idx], printable_area);

        if (!stable_interface.empty()) {
            if (interface == nullptr) {
                interface = allocate_like(intermediate, SupporLayerType::TopInterface);
                interface_layers[layer_idx] = interface;
            }
            interface->polygons = union_(interface->polygons, stable_interface);
        }

        Polygons stable_base_interface;
        if (!base_interface_targets[layer_idx].empty()) {
            stable_base_interface = intersection(base_interface_targets[layer_idx], printable_area);
            if (interface != nullptr && !interface->polygons.empty())
                stable_base_interface = diff(stable_base_interface, interface->polygons);
        }

        if (!stable_base_interface.empty()) {
            if (base_interface == nullptr) {
                assert(!base_interface_layers.empty());
                base_interface = allocate_like(intermediate, SupporLayerType::Base);
                base_interface_layers[layer_idx] = base_interface;
            }
            base_interface->polygons = union_(base_interface->polygons, stable_base_interface);
        }

        if (interface != nullptr && !interface->polygons.empty()) {
            intermediate.polygons = diff(intermediate.polygons, interface->polygons);
            if (base_interface != nullptr)
                base_interface->polygons = diff(base_interface->polygons, interface->polygons);
        }
        if (base_interface != nullptr && !base_interface->polygons.empty())
            intermediate.polygons = diff(intermediate.polygons, base_interface->polygons);
    }
    timing.finish(layer_count);
}
    ...
    SupportGeneratorLayerStorage      &layer_storage)
{
    ...
    SupportProfileStage total_timing("support-interface", "Generate all interface geometry", intermediate_layers.size());
    ...
    if (! intermediate_layers.empty() && support_params.has_interfaces()) {
        ...
        std::vector<Polygons> top_interface_targets(intermediate_layers.size());
        std::vector<Polygons> top_base_interface_targets(intermediate_layers.size());
        ...
        auto insert_layer = [&layer_storage, smooth_supports, closing_distance, smoothing_distance, minimum_island_radius_top, minimum_island_radius_bottom](
                ...
                const Polygons *subtract, SupporLayerType type, Polygons *top_target) -> SupportGeneratorLayer* {
        ...
        SupportProfileStage projection_timing("support-interface", "Project top and bottom contact footprints", intermediate_layers.size());
        tbb::parallel_for(tbb::blocked_range<int>(0, int(intermediate_layers.size())),
            [&bottom_contacts, &top_contacts, &top_interface_layers, &top_base_interface_layers, &intermediate_layers, &insert_layer, &support_params,
             snug_supports, &interface_layers, &base_interface_layers, &top_interface_targets,
             &top_base_interface_targets](const tbb::blocked_range<int>& range) {
                ...
                for (int idx_intermediate_layer = range.begin(); idx_intermediate_layer < range.end(); ++ idx_intermediate_layer) {
                    ...
                    if (! polygons_bottom_contact_projected_interface.empty() || ! polygons_top_contact_projected_interface.empty() ||
                        ...
                        interface_layer = insert_layer(
                            ...
                            interface_type == SupporLayerType::TopInterface ? &top_interface_targets[idx_intermediate_layer] : nullptr);
                    ...
                    if (! polygons_bottom_contact_projected_base.empty() || ! polygons_top_contact_projected_base.empty() ||
                        ...
                        base_interface_layers[idx_intermediate_layer] = insert_layer(
                            ...
                            &top_base_interface_targets[idx_intermediate_layer]);
                }
            });
        projection_timing.finish(intermediate_layers.size());

        if (smooth_supports) {
            stabilize_top_interface_footprints(
                intermediate_layers, interface_layers, base_interface_layers,
                top_interface_targets, top_base_interface_targets,
                coord_t(std::ceil(smoothing_distance)), layer_storage);
        }
        ...
        SupportProfileStage merge_timing("support-interface", "Merge and remove empty interface layers", interface_layers.size() + base_interface_layers.size());
        ...
        merge_timing.finish(interface_layers.size() + base_interface_layers.size());
        BOOST_LOG_TRIVIAL(debug) << "PrintObjectSupportMaterial::generate_interface_layers() in parallel - end";
    }

    total_timing.finish(base_and_interface_layers.first.size() + base_and_interface_layers.second.size());
...
SupportGeneratorLayersPtr generate_raft_base(
    ...
    SupportGeneratorLayerStorage      &layer_storage)
{
    SupportProfileStage total_timing("support-raft", "Generate all raft geometry", slicing_params.raft_layers());
    SupportProfileStage brim_timing("support-raft", "Build raft brim exclusion", object.layer_count());
    ...
    brim_timing.finish(brim.size());

    SupportProfileStage geometry_timing("support-raft", "Build raft base and interface layers", slicing_params.raft_layers());
    ...
    geometry_timing.finish(raft_layers.size());
    total_timing.finish(raft_layers.size());
    return raft_layers;
}
...
void tree_supports_generate_paths(
    ...
    const SupportParameters &support_params)
{
    SupportProfileStage timing("support-wall", "Generate tree branch walls", polygons.size());
    ...
    for (ExPolygon& expoly : closing_ex(polygons, float(SCALED_EPSILON), float(SCALED_EPSILON + 0.5 * flow.scaled_width()))) {
        ...
        const size_t requested_wall_count = support_params.tree_support_wall_count > 0 ?
            support_params.tree_support_wall_count :
            (expoly.area() > support_params.tree_branch_diameter_double_wall_area_scaled ? 2 : 1);

        for (size_t wall_idx = 1; wall_idx < requested_wall_count; ++wall_idx) {
            ExPolygons next_regions = offset2_ex(regions_to_draw_inner_wall, -1.5 * flow.scaled_width(), 0.5 * flow.scaled_width());
            if (next_regions.empty())
                break;

            if (!eec) {
                eec = std::make_unique<ExtrusionEntityCollection>();
                // Keep all loops of a branch together and print them from inside to outside.
                eec->no_sort = true;
            }
            for (const ExPolygon &region : regions_to_draw_inner_wall)
                extrusion_entities_append_paths(
                    eec->entities, draw_perimeters(region, clip_length), ExtrusionRole::erSupportMaterial,
                    flow.mm3_per_mm(), flow.width(), flow.height(),
                    // Disable reversal of the path, always print CCW.
                    false);
            regions_to_draw_inner_wall = std::move(next_regions);
        }
...
{
    SupportProfileStage total_timing("support-toolpath", "Generate support walls and base infill", polygons.size());
    if (polygons.empty())
        return;
    ...
    if (with_sheath) {
        ...
        SupportProfileStage infill_timing("support-infill", "Generate support base infill", polygons.size());
        fill_expolygons_generate_paths(dst, closing_ex(polygons, float(SCALED_EPSILON)), filler, fill_params, density, role, flow);
        infill_timing.finish(dst.size());
        total_timing.finish(dst.size());
        return;
    }
    ...
    for (ExPolygon &expoly : closing_ex(polygons, float(SCALED_EPSILON), float(SCALED_EPSILON + 0.5*flow.scaled_width()))) {
        ...
        const size_t wall_count = std::max<size_t>(1, support_params.support_wall_count);
        ExPolygons perimeter_regions{expoly};
        SupportProfileStage wall_timing("support-wall", "Generate normal support walls", wall_count);
        for (size_t wall_idx = 0; wall_idx < wall_count && !perimeter_regions.empty(); ++wall_idx) {
            for (const ExPolygon &region : perimeter_regions)
                extrusion_entities_append_paths(
                    out, draw_perimeters(region, clip_length), ExtrusionRole::erSupportMaterial,
                    flow.mm3_per_mm(), flow.width(), flow.height());
            if (wall_idx + 1 < wall_count)
                perimeter_regions = offset_ex(perimeter_regions, -spacing);
        }
        wall_timing.finish(wall_count);
        // Fill in the rest.
        const double infill_offset = (double(wall_count) - 0.6) * spacing;
        SupportProfileStage infill_timing("support-infill", "Generate support base infill", 1);
        fill_expolygons_generate_paths(out, offset_ex(expoly, float(-infill_offset)), filler, fill_params, density, role, flow);
        infill_timing.finish(out.size());
...
{
    SupportProfileStage timing("support-interface", "Generate contact loops", top_contact_layer.empty() ? 0 : top_contact_layer.polygons_to_extrude().size());
    ...
    timing.finish(top_contact_layer.extrusions.size());
}
...
{
    SupportProfileStage total_timing("support-toolpath", "Generate all shared support toolpaths", support_layers.size());
...
//    const coordf_t link_max_length_factor = 3.;
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, n_raft_layers),
        [&support_layers, &raft_layers, &intermediate_layers, &config, &support_params, &slicing_params,
            &bbox_object, &throw_on_cancel, link_max_length_factor]
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id)
        {
            ...
            SupportProfileStage raft_layer_timing("support-raft-toolpath", "Generate raft layer toolpaths", raft_layer.polygons.size());
            ...
            raft_layer_timing.finish(support_layer.support_fills.entities.size(),
                "support_layer=" + std::to_string(support_layer_id));
        }
    });
    ...
    SupportProfileStage numbering_timing("support-interface", "Number top and bottom interface layers", support_layers.size());
    {
    ...
    numbering_timing.finish(support_layers.size());
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        [&config, &slicing_params, &support_params, &support_layers, &bottom_contacts, &top_contacts, &intermediate_layers, &interface_layers, &base_interface_layers, &layer_caches, &loop_interface_processor,
            &bbox_object, &angles, &top_interface_numbers, &bottom_interface_numbers, &throw_on_cancel, n_raft_layers, link_max_length_factor]
            (const tbb::blocked_range<size_t>& range) {
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id)
        {
            ...
            const float   support_interface_angle = (config.support_interface_pattern == smipRectilinearInterlaced) ?
                ...
                ((support_params.support_style == smsGrid || config.support_interface_pattern == smipRectilinear || config.support_interface_pattern == smipTriangles) ?
                support_params.interface_angle : support_params.raft_interface_angle(support_layer.interface_id()));
            const std::string layer_diagnostic = "support_layer=" + std::to_string(support_layer_id);
            SupportProfileStage prepare_layer_timing("support-toolpath", "Prepare support layer regions", 1);
            ...
            prepare_layer_timing.finish(1, layer_diagnostic);
            ...
            auto extrude_interface = [&](SupportGeneratorLayerExtruded &layer_ex, InterfaceLayerType interface_layer_type) {
                const char *stage_name =
                    interface_layer_type == InterfaceLayerType::TopContact ? "Generate top contact toolpaths" :
                    interface_layer_type == InterfaceLayerType::BottomContact ? "Generate bottom contact toolpaths" :
                    interface_layer_type == InterfaceLayerType::RaftContact ? "Generate raft contact toolpaths" :
                    interface_layer_type == InterfaceLayerType::Interface ? "Generate interface toolpaths" :
                    "Generate interface-as-base toolpaths";
                SupportProfileStage interface_timing("support-interface-toolpath", stage_name,
                    layer_ex.empty() ? 0 : layer_ex.polygons_to_extrude().size());
            ...
            SupportProfileStage base_interface_timing("support-interface-toolpath", "Generate base interface toolpaths",
                base_interface_layer.empty() ? 0 : base_interface_layer.polygons_to_extrude().size());
            ...
            SupportProfileStage base_timing("support-base-toolpath", "Generate base walls and infill",
                base_layer.empty() ? 0 : base_layer.polygons_to_extrude().size());
            ...
            base_timing.finish(base_layer.extrusions.size(), layer_diagnostic);

            // Merge base_interface_layers to base_layers to avoid unneccessary retractions
            SupportProfileStage finalize_layer_timing("support-toolpath", "Merge paths and build support islands", layer_cache.nonempty.size());
            ...
            finalize_layer_timing.finish(support_layer.support_islands.size(), layer_diagnostic);
        } // for each support_layer_id
    });

    // Now modulate the support layer height in parallel.
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        [&support_layers, &layer_caches, &support_params, &bbox_object, &throw_on_cancel]
            (const tbb::blocked_range<size_t>& range) {
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id) {
            if (throw_on_cancel)
                throw_on_cancel();
            ...
            LayerCache   &layer_cache   = layer_caches[support_layer_id];
            const std::string layer_diagnostic = "support_layer=" + std::to_string(support_layer_id);
            SupportProfileStage modulation_timing("support-toolpath", "Modulate overlapping support extrusion", layer_cache.nonempty.size());
            ...
            modulation_timing.finish(support_layer.support_fills.entities.size(), layer_diagnostic);

            // Orca: Generate iron toolpath for contact layer
            SupportProfileStage ironing_timing("support-interface-toolpath", "Generate support interface ironing", layer_cache.polys_to_iron.size());
            ...
            ironing_timing.finish(support_layer.support_fills.entities.size(), layer_diagnostic);
        }
    });
...
#endif // NDEBUG
    total_timing.finish(support_layers.size());
}
