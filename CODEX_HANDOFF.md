...
- **Next step — branch placement by angular sector, which now owns both remaining residues.** What is left on the hollow gear is 1.4656 mm2 in one compact blob at radius 17.4 where the angular half-gap (3.9 mm) is well inside micro reach (6.4 mm): a sector with **no branch**, not a fan gap. snug overhang's 1.84 mm2 is the same shape of defect. Branch placement is currently a by-product of enumerating trunk source turns, so nothing computes which angular sectors get served before routing — the trunk-stage principle of §7.0.1 applied one level down. Do not attack either residue from the micro side; micro planning does not fail on these fixtures any more.
...
That makes the last hollow gear residue the same defect as snug overhang's 1.84 mm2: a sector a rigid parallel branch cannot be placed into from any available source turn. **Both belong to the branch stage, not to Micro Branch.** Micro planning itself no longer fails on this fixture at all. The open question for the next step is how branch placement decides which angular sectors get served — today it is a by-product of enumerating source turns, so nothing computes sector coverage before routing, which is the same "decide reach before routing" principle §7.0.1 states for the trunk.
