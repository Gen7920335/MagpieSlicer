...
- **Next step — the gain-versus-outcome discrepancy.** At two branches per turn with micro **off**, hollow gear target 1 gains two branches and its uncovered area does not move by a single digit. The greedy loop only accepts a branch on positive estimated gain, so a branch that changes the outcome by nothing means the gain estimate and the outcome disagree — the same shape as every entry in the defect table, and now with a cheap reproduction: flip `maximum_branches_per_source_turn` to 2 unconditionally and compare `branches` against `uncovered_mm2` on the macro-only fixture. snug overhang's 1.84 mm2 is unaffected by any of this and stays collision-limited under parallelism.
