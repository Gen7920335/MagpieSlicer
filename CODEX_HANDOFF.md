...
- **Beta 2 release authorized (2026-09-10):** User requested installer creation and beta push. Target is the existing `beta/2.5.0.0.9` branch and a new `v2.5.0.0.9-beta.2` GitHub prerelease; do not overwrite Beta 1. Package the latest non-GUI-verified source with a freshly compiled Korean runtime catalog. Actual installation and GUI operation remain user-owned. Release notes: `release-notes-2.5.0.0.9-beta.2.md`. Preserve all backup/artifact/untracked material outside the three owned source headers; no portable package or physical-printer action.
