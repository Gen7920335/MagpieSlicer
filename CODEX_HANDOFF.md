...
- **Next verified step — bound each branch's credited coverage by the micro reach available from its own ring.** With micro enabled the hollow gear residue falls from 55.81/65.49 to 9.83/15.32 mm2 (covered fraction 79.2 % → 94.5 %) and then fails at `micro_tree_geometry`, localised to `plan_seeded_micro_tree`'s `tree_start_layer` search: a branch born at layer 29 of 58 can reach 2.24 mm while the region credited to it extends 4.32 mm. `bridgeable()` credits by distance alone, ignoring how much height remains above the branch. Micro Branch itself is neither a rewrite nor open-ended verification — it already fills the fan gaps. Numbers and the full diagnosis in §7.0.2, "Micro Branch already closes most of the residue". The fixtures stay macro-only by decision, so this changes no test outcome today; verify against `terminal ring feeds local tree tips` plus scratch runs with micro forced on. Both are named with measurements. Do not start from the reported failure-stage names: `OverlappingTrunks` is two steps from its cause and there is only one trunk in that forest.
