...
- **Beta 2 published (2026-09-10):** Product commit `6e98411bb44fb0fef6679e7f7072337f30db4df2` is tagged `v2.5.0.0.9-beta.2` and published as a GitHub prerelease; Beta 1 was preserved. Installer: `build-vulkan/installer/20260910-115039/MagpieSlicer_Windows_Installer_V2.5.0.0.9_x64.exe`, 142,434,749 bytes, SHA-256 `6E6EBC1FDAB8223345389678DCFDA732D776B8526F92C75B4E1A065FD1AF33F3`. GitHub asset size/digest match local. NSIS 15,250-file test/extraction passed; packaged EXE/DLL and freshly compiled Korean MO match build inputs, executable versions are `2.5.0.0.9`, extracted CLI help exits 0. Actual installation/GUI and physical printing were not run. Release: `https://github.com/Gen7920335/MagpieSlicer/releases/tag/v2.5.0.0.9-beta.2`. The branch may contain a later documentation-only checkpoint commit; the release tag intentionally remains on the product commit used for the package.

- **Latest non-GUI checkpoint (2026-09-10):** User owns actual GUI verification. Product/FFF build passed (non-gui-final-build.log), core10/838 and CLI340+8+16=364 passed;364 settings unchanged,16archives/80PNG checked. Latest full Bunny60 single-nozzle towerON579295 model moves wrongtarget0 and prior command SHA unchanged. BUG014 fixed per-Flow fixed L band/common participating-tool area/hottest used material; qualified .2/.8 both-order Bunny60 gate7445 passed with0 spacing/area failures. Added imported height-range missing layer_height inheritance: actual3MF baseline null crash, corrected regression passed. BUG006 diagnostic now CRITICAL and explicitly says retry may leave unsupported areas; PERMANENT DIAGNOSTIC in PrintObject.cpp. Geometry recovery remains UNKNOWN. BUG005 cross-channel overlap not reproduced in8 Bunny60conditions; normal-only tiny overlap matches Mixed-OFF control. Read m/outputs/bugfix-20260910/NON_GUI_RESULT.md and WORK_STATUS.md; GUI_USER_CHECKLIST.md is user-owned. Remaining: actualtree-failure Bunny60 fallback contact coverage, dynamic wall-tool/Auto/purge leads, Vulkan spatial work/kernel/host totals. No all34-cleared/zero-bug claim,4070SUPER qualification,GUI clicks,physical printer commands,installer,commit orpush. No active builds/tests at checkpoint. Earlier pause/GUI-access restrictions below are superseded by user resume and GUI-user ownership.

- **2026-09-10 bug-fix task RESUMED:** User resumed non-GUI fixes and verification; actual GUI testing is user-owned, do not attempt GUI automation. Current step: reproduce BUG005 Mixed cross-Z extrusion overlap on full Bunny60 before changing geometry. Branch beta/2.5.0.0.9, HEAD dbf73327dbe68dfb1275621675973eb46f8dbd00; preserve original dirty7, all untracked and verified1843-file before backup. Authoritative detailed checkpoint: `C:/Users/svadmin/Documents/Codex/2026-08-31/m/outputs/bugfix-20260910/WORK_STATUS.md` and `MAGPIE_FIX_AND_RETEST_2026-09-10.md`, latest pause section supersedes historical pending entries. BUG021 now passed actual cereal Undo/Redo424 checks, core50/6544, product build, CLI config340/reload8/save16 and full Bunny60 towerOFF unchanged commands/579295 model extrusions wrong target0. These precede BUG019. BUG019 now protects all15 numeric edits against polling/stale ACK/missing status; valid original baseline20 failures, final Snapmaker23/328 and actual adapter body headless115 checks passed; native GUI/server behavior not verified. Latest product numeric-input-final-product-build.log succeeded. Latest existing CLI save16 outputs verified: settings16/project payload16 unchanged, archive CRC and PNG80 header/dimensions valid (numeric-input-save-gate-summary.json). No extra execution launched during pause closeout. Earlier fixes/gates remain documented, not all34 investigations cleared. Remaining primary candidates BUG005/006 Mixed geometry and BUG014 multi-nozzle temperature tower; shared-nozzle UX, GUI runtime and additional wall-tool/purge leads still open. Vulkan spatial work/kernel/host totals remain unmeasured. GUI official launch blocked GetCursorPos0x80070005 with svadmin Disc; no clicks/permission bypass. Isolated gui-runtime-final DLL predates BUG019. Host GT1030, not4070 SUPER qualification. No physical printer command, installer, commit or push. User has now authorized resume; proceed with remaining candidate reproduction, never claim zero bugs.

- **Full support timing coverage (2026-09-01, not packaged):** Detailed profiler instrumentation now spans Normal, Cura, Classic Tree, Organic Tree, Mixed and Resin support from demand/overhang analysis through projection/propagation, top and bottom contacts, interface stacking, raft geometry/toolpaths, support walls, base infill, overlap modulation/ironing, layer assembly and final support toolpaths. The seven instrumented translation units currently contain 113 timed scopes; layer-level events carry object/support layer diagnostics and stage-specific work-item counts where available. Revision 4 counts every completed event in exact per-stage summaries even after the 250,000 raw-timeline limit; `dropped_events` now means summarized-only raw events, while `counted_events`, totals, distributions, work items and diagnostics remain complete. It also exports CPU wall, GPU wall and their actual interval overlap in `backend_wall_us`; Hybrid-labelled time stays separate because its device/host split is ambiguous. A limit-3 standalone harness verified raw=3, summarized-only=2, counted=5, work-items=50 and diagnostic counts 3/2. Nested and parallel `summed_task_us` values are task time and are not additive wall time; use event intervals/covered wall time for elapsed attribution. `SupportTiming.hpp` compiles to a no-op without `MAGPIE_SLICING_TIMING`. Release GUI target rebuilt successfully and `verify_slicing_profiler_contract.ps1` passed. Final development `MagpieSlicer.dll`: 130,364,928 bytes, SHA-256 `7F3AA31B30E3753F220008E15979AD00AB6FBACFAE6A8EB066D75171C1BC860E`. No actual slice/profile JSON, Stanford Bunny/60 run, GUI interaction, installer, commit or push was performed for this change. Plan: `m/outputs/Magpie-CUDA-Full-Power-Plan-2026-09-01.md`; timing catalog: `m/outputs/Magpie-Support-Detailed-Timing-2026-09-01.md`.

- **Support bottleneck analysis (2026-09-01, read-only):** Confirmed server 3MF is 52,716-triangle Bunny, ~118.425mm Z, layer_height=0.04 (~2,960 object layers), Mixed+Cura/Tree Hybrid, coverage threshold 100%, selective merge on, Cura join 2mm, expansion 3mm, XY gap 0, geometry resolution 0.01mm, independent support height on. Normal support walls=0/tree walls=1, so object wall_loops=10 is not the direct support wall count. Source structure: Cura serially propagates/stores polygons down object layers; Mixed retains Cura layers, converts their actual extrusion envelopes into per-object-layer Tree obstacles, generates Tree separately, then merges; legacy Tree quantizes branch radii at 0.2mm and precomputes/caches collision and cumulative avoidance ExPolygons for each used radius through its highest object layer. 0.4..10mm permits up to ~49 radius buckets; ~2,960 layers gives a theoretical ~145k pair upper bound, not an observed job count. This explains a plausible time+memory multiplication and is consistent with secondary ~27GB memo, but exact dominant substage remains UNKNOWN because supplied timing JSON lacks Mixed/Cura/Tree inner records. Existing source already logs Mixed/Cura/Tree stages and MAGPIE_TREE_PRECOMPUTE; obtain original app log or run the required Bunny/60 counterfactual before attributing savings. Uploaded project angle is 75, not 60. CUDA is not the primary support cause: Off support is still 480.567s and actual CUDA events overlapping support are ~3ms. Report: m/outputs/Magpie-Support-Bottleneck-Analysis-2026-09-01.md. No product edit/build/new slice/package/push in this analysis.

- **New matched server pair (2026-09-01):** C:/server now has cuda3mf.3mf + cuda.json and noncuda3mf.3mf + misspelled noncude.json. Both profiles completed on RTX 4070 SUPER/16 threads. Their model and project_settings.config hashes match; only plate_1.json differs (transient object id 120/121 and recorded first-layer estimate). CUDA 634.920021s versus Off 523.198161s: CUDA slower by 111.721860s / 21.35%. Perimeters 9.755953s vs 5.535923s (+76.23%); support 585.508006s vs 480.567135s (+21.84%). Strict mode accepted and CPU-rechecked 64,487,896 results with zero failures; 761.208ms summed device kernels, 111,181.933ms cumulative host call time. Do not equate cumulative concurrent host time with elapsed time. This is one CUDA-then-Off pair, not a repeated/interleaved qualification. Uploaded project is Bunny, 0.04mm, wall 10, Mixed+Cura/Hybrid, but support_threshold_angle=75, NOT the required 60-degree support test. All four filament_type entries are PLA (colors differ), so the 3MF does not by itself prove stale material text or identify a before/after material type. Report: m/outputs/Magpie-4070-CPU-CUDA-Pair-2026-09-01.json.

- **Latest UI/debug work (2026-09-01, not packaged):** User reported stale material text after Toolhead changes, disabled support-interface material selector, and very slow slicing. `Plater.cpp` now refreshes dynamic material labels on filament-type changes and after the preset-selection transaction; `ConfigManipulation.cpp` keeps interface-material selection editable without enabling support/interface geometry. Release GUI target compiled/linked successfully in work/magpie-timing-build; no GUI automation or runtime UI reproduction. Exact stale-text cause remains UNKNOWN: current lists display material type, not full preset name, and the user has not supplied before/after names. Existing 0.9 installer does NOT contain these changes; no version bump, package, install, commit or push. Source and prior 0.9 EXE/DLL/PDB backup: work/ui-support-before-20260901-001631. Report: m/outputs/Magpie-UI-and-Slow-Slicing-2026-09-01.md.
- **Server performance evidence / next verified step:** C:/server contains the verified original 0.9 installer, timing JSON and secondary diagnostic notes, but no 3MF or original app log. User's RTX 4070 SUPER CUDA session completed in 710.617851s: support 654.390355s (92.1%), G-code export 43.452232s, perimeters 6.092921s. Only 2 actual CUDA events (3.027ms host, 1.340096ms device) overlap support; most CUDA execution is Arachne walls. 45,635,616 results accepted, zero comparison failures. Cumulative queue waits are not elapsed time. No CPU control or internal support-stage timing in this JSON: do not claim speedup/regression or a specific support root cause. Obtain the exact slow 3MF and/or app log (`Mixed support timings` already emitted by source), plus material before/after names; preserve quality settings. No new slicing or geometry optimization in this UI task. See m/outputs/Magpie-Server-Timing-2026-09-01.json. Secondary RADAR/memory notes do not prove a crash or its cause; keep separate from the completed profile.

- **Latest local installer (2026-08-31):** 2.5.0.0.9, Windows 2.5.0.9, internal 2.5.0-modified.0.9; version.inc is the owner. CUDA/Vulkan/timing enabled, normal Magpie identity. Built by work/build-installer-09.cmd in work/magpie-timing-build. Installer: C:/Users/svadmin/Documents/Codex/2026-08-31/m/outputs/MagpieSlicer_Windows_Installer_V2.5.0.0.9_x64.exe (142181015 bytes, SHA-256 0FB2A98603BD799A6561354EA9E616B47B2744A708220649A9F7EA02854204FE). NSIS archive test/extraction, 40 packaged binary input hashes (Release + declared MSVC redistributables), 15198 resources, 16 new Korean translations, EXE metadata and extracted CLI --help passed. CLI help is not slicing or installed-GUI verification. Old 0.8 installer/Release/original KO catalog preserved. No installation, GUI automation, new Bunny matrix, actual 4070 test, commit/tag/push/release. Records: m/outputs/Magpie-2.5.0.0.9-Installer.md and Installer-Verification.json. Packaging uses work/installer-09-catalog.cmake to update only the new CPack stage; keep this hook/catalog when reproducing the local package.
- **Current scope:** Optional CUDA implementation for RTX 4070 SUPER. User authorizes implementation and CPU/CUDA comparison. Latest criterion: Tree output may vary if support function/settings remain correct; whole Tree G-code identity is NOT a prerequisite. Do not restart CPU Tree reproducibility investigation as a global blocker. Preserve cumulative dirty source and original 2.5.0.0.8 artifacts, Tsunami removal, stable Mixed/Resin enums, wall/nozzle/flow semantics.
- **Implemented:** Five CUDA kernels: exact vertical intersections; integer AABB candidates; FP64 point-segment distance; mesh layer-range binary searches; point-in-polygon containment. Reusable bounded Driver API runtime, pinned staging, owned stream, primary context, full result validation, whole-batch fallback. Native sm_61/sm_89 compiled. Default OFF. GUI Off/On added; CLI enable via MAGPIE_CUDA_SLICER_ENABLE=1; FORCE_DISPATCH=1 is an experimental small-batch override. MAGPIE_CUDA_DISABLE_OPERATIONS selects per-operation exclusions, not an automatic speed selector.
- **Callers:** Rectilinear model/support sweeps; Arachne/Classic/seam/Gyroid/Lightning AABB; Lightning distances; TriangleMeshSlicer layer ranges; Cura corresponding-layer XY-gap AABB; Tree/Mixed inner contact candidate containment. CPU retains polygon boolean/offset, triangle intersection/loop assembly, actual wall/flow/nozzle decisions, support demand/propagation, tree node insertion/merge/link decisions and G-code. Resin uses common mesh/infill CUDA; its ray tracing and tree construction were not ported.
- **GUI boundary (latest):** CUDA Off/On selectors now exist beside Vulkan in BBLTopbar and Preferences > Graphics. AppConfig owns mutually exclusive persisted choices; BackgroundSlicingProcess snapshots them on the UI thread before waking the worker. GUI Off overrides environment enable; GUI On does not inherit CLI force-dispatch. Existing Vulkan Tree/Mixed guard remains Vulkan-only. GUI implementation was not interactively tested; the latest 0.9 package is described above. No actual installation, commit or push was performed.
- **Timing settings (latest):** Preferences > Graphics: Off / Major stages / Detailed and opt-in automatic completed-FFF JSON save to app-data/slicing-timing. Timer overlay has details and export. Schema revision 4 adds complete aggregate counting beyond the bounded 250,000 raw timeline; revision 3 fields for duration distributions, diagnostic counts, CUDA transfer bytes, host serialization wait and launch/wait times remain. `dropped_events` is summarized-only rather than uncounted work. Null means unmeasured; overlapping task/phase durations are not additive. JSON export I/O is outside session duration. New Korean PO/POT and separate MO verified (16 strings); active development resource junction still points at preserved original resources, so development-tree new strings may display in English. The existing 0.9 installer predates revision 4 and the support-detail additions.
- **Verified development build:** work/magpie-timing-build (not original build-vulkan). Final GPU self-test passes independent intersection reference, overflow/degenerate rejection, changing-input 1000 batches, concurrent context reuse, AABB exhaustive oracle/chunking/caps, distance/mesh/containment boundaries and injected whole-batch rejection. Product reconnect_polylines test verifies a join after the original AABB expands. CUDA OFF compiles/runs without CUDA SDK headers/libraries. Self-test now links product geometry and needs src/Release on its process PATH for existing dependent DLLs.
- **Earlier expanded-kernel Bunny/60 results (before UI/timing settings change):** Eight configurations, 16 CPU/CUDA CLI executions passed: Gyroid/Arachne, Lightning/Classic, Normal extra-overhang walls, Cura, Tree, Mixed, Resin Default and Branching. 22,176,917 GPU results accepted; zero GPU comparison failures. Completed sessions, finite G-code, extrusion roles and configured widths checked. Six support interface swath comparisons at 0.25mm grid/0.125mm sampling give 100% retention/Jaccard; Mixed has only 30 sampled interface cells, so this is not a broad support-coverage proof. Whole support connectivity/collision adequacy and physical printing remain unverified.
- **Supplemental evidence/limitations:** Lightning with point_segment_distance excluded executes AABB distance-field CUDA (60,705 accepted); Tree branch diameter 1mm pair also passed (301,182 accepted). Both are separate from the eight-case totals. point_in_polygon CUDA execution was NOT observed in the default or 1mm Bunny runs; only its real-GPU self-test and caller wiring are verified. Classic candidate calls likewise may have no eligible work in the Bunny matrix. Do not report enum names/call counts as actual dispatch evidence. Latest Tree run records 774 containment requests resolved on CPU because geometry was empty; no containment device execution is claimed for that run.
- **Known failures / DO NOT REPEAT:** Additional Bunny X90 rotation exited 0xC0000005 in the CUDA-OFF development baseline before any CUDA comparison. Root cause unverified; do not label it pre-existing or CUDA-caused. Keep separate from the eight passing configurations. Prior pageable HtoD/default-stream vs nonblocking-kernel ordering bug is fixed: retain pinned buffers and same-stream HtoD/kernel/DtoH completion. Do not use global cudaDeviceReset, mandatory startup linkage to nvcuda.dll, or partial results. Reconnection now stops filtering by the original AABB after base extension.
- **Performance / next work:** Actual host is GT1030, not 4070 SUPER. Earlier expanded-kernel strict full-reference single pairs were 0.35–7.17% slower overall on GT1030; this does not select a 4070 winner. Measure repeated, interleaved end-to-end runs on actual 4070 before enabling faster paths. CUDA primitives are implemented, not a full-GPU slicer or qualified Auto mode. Full strengthened final-candidate 28-case Bunny/60 matrix, nozzle/wall/painting/cancel regression, actual GUI/installed behavior and full support-function checks remain open. Historical 393 CTest/34 Resin/12 Core are not new CUDA evidence.
- **Pre-0.9-version settings records:** m/outputs/Magpie-CUDA-Settings-2026-08-31.md and Settings-Verification JSON; source backup work/cuda-settings-before-20260831-155948 (19 files). Timing tests 12/126, real AppConfig save/load and GPU self-tests, CUDA-disabled build, GUI-inclusive build passed. Four final-build Normal/Gyroid and Tree CPU/CUDA Bunny60 CLI runs passed; zero comparison failures. CLI FORCE_DISPATCH=1 was used; this is not GUI-default performance qualification. Plater now clears derived caches on the next idle slicing request after acceleration/detail choices change; original model and G-code-only plates preserved. Cache GUI path is compiled/statically reviewed, not GUI-interaction tested. Original installer, Release and version hashes preserved.
- **Earlier records:** m/outputs/Magpie-CUDA-Expanded-Implementation-2026-08-31.md, Expanded-Results JSON, Interface-Comparison JSON, Supplemental JSON and Runbook. Source backup work/cuda-expand-before-20260831-145736; earlier work/magpie-cuda-before-20260831 and cuda-step1-before-20260831-142323 preserved. Initial 73-item manifest has no unexpected changes outside owned edits. Original installer and Release EXE/DLL hashes and version.inc preserved. Everything below is historical; do not treat old Tsunami or old Tree identity gates as current instructions.
...
- 관련 파일: `src/libslic3r/Gpu/VulkanSlicer.cpp/.hpp`, `GpuExactGeometry.hpp`, compute shaders, `PerimeterGenerator.cpp`, `Support/TreeSupport.cpp`, `Print.cpp`, top-level CMake. Cura support keeps its same-layer AABB preflight on CPU to avoid an all-layer candidate batch.
...
- Confirmed code locations dispatch exact scanline work and conservative non-topological AABB candidates from perimeter/fill paths. Tree/Mixed support remains CPU-authoritative for deterministic topology, and Cura support uses a same-layer CPU AABB preflight.
- Exact contract:
...
- 2026-08-26 1차 수정: Cura는 같은 레이어 support/model AABB만 필요하지만 기존 코드는 전 레이어 후보 배치를 만든 뒤 `query == target`만 소비했다. 이 경로를 보수적 CPU same-layer AABB preflight로 교체해, 겹칠 수 없는 레이어의 XY-gap polygon offset도 지연/생략한다.
- 2026-08-26 1차 수정: Mixed+Cura가 채널 판정과 실제 생성에서 자동/강제 overhang을 두 번 검출하던 경로를 precomputed channel 재사용으로 변경했고, base-layer 변환의 support polygon deep copy를 move로 교체했다.
- 2026-08-26 검증: `build-vulkan` Release 빌드 성공, focused `[SupportMaterial]` 47 cases/6269 assertions 통과, 전체 CTest 380/380 통과.
- 2026-08-26 Stanford Bunny 60-degree: pure Cura triangles 실슬라이스 통과(544 support layers, 400 interface layers), 이전 동일 설정 결과와 normalized G-code SHA-256 `8A03331120850CFEBBA6C803CDCCD24D2F2EF3A674C1379382499D6280E01D4D` 일치. Mixed의 Prusa/organic, Prusa/tree_hybrid, Cura/tree_strong 세 조합도 모두 통과.
- 2026-08-26 Vulkan isolation: Cura grid/snug을 CPU/GPU 각 1회(총 4 slices) 비교해 canonical/segment/coverage/metrics hash가 모두 일치했고 `cura-support-spatial` dispatch는 0회였다. `verify_vulkan_integration_contract.ps1`의 코드 계약은 통과했으나 로컬에 sealed CPU baseline manifest가 없어 스크립트 전체 상태는 1 failure로 남는다.
- 성능 판정: pure Cura Bunny의 기존/신규 실측은 약 15-17초 범위로 유의한 개선을 입증하지 못했다. 같은 레이어 bbox가 대부분 겹치는 모델에서는 preflight 이득이 작을 수 있다. per-stage timing은 아직 실행하지 않았다.
...
4. What is the exact current Vulkan production scope?
   - Known: Cura's one-to-one same-layer AABB check intentionally stays on CPU; perimeter and remaining non-topological candidate call sites still need periodic enumeration against runtime stats.
...
# 19. 2026-08-27 Legacy Tree / Mixed Performance Pass

- Scope: legacy Tree Slim/Strong/Hybrid path in `src/libslic3r/Support/TreeSupport.cpp`; Organic remains on `TreeSupport3D`.
- Accepted profiler fix:
  - profiler ownership is per `TreeSupport` instance and shared with its preview cache;
  - fine-grained cache counters and durations are atomic/thread-safe;
  - set `MAGPIE_TREE_PROFILE=1` to emit `MAGPIE_TREE_PRECOMPUTE` and `MAGPIE_TREE_PROFILE` diagnostics to stderr.
- Accepted optimization:
  - collect the highest required object layer per quantized branch radius;
  - flatten independent collision `(radius, layer)` jobs and finish them in parallel first;
  - then build avoidance bottom-up once per radius;
  - prewarm radius-zero collision only on active layers, matching the legacy downstream demand;
  - do not restore parallel node mutation, the removed Vulkan AABB shortcut, or worker-timing-dependent topology.
- Exact A/B, Stanford Bunny, 0.4 mm nozzle, 60 degree threshold, 0.20 mm layer, Mixed(auto), threshold 50%, Selective Merge ON, Tree Hybrid:
  - baseline Tree: `50.018 s`; optimized Tree: `39.646 s` (`20.7%` faster);
  - baseline whole CLI: `154.753 s`; optimized whole CLI: `142.197 s` (`8.1%` faster);
  - collision duplicate calculations: `0` before and after;
  - G-code diff: 2,366 changed lines, all timestamp or object-id comments; extrusion/motion differences: `0`.
- Required actual-slice matrix passed 12/12:
  - Stanford Bunny, 0.4 mm nozzle, threshold angle 60 degrees;
  - layer height `0.20` and `0.08`;
  - Mixed coverage threshold `0/50/95%`;
  - Selective Merge `OFF/ON`;
  - all cases generated finite G-code with embedded settings and no critical diagnostics; Tree profile was present whenever threshold was greater than zero.
- Rejected optimization:
  - caching `union_ex(extra_obstacles[layer])` once in `set_extra_obstacles()` improved the 0.08/95%/merge-ON stress case by only `6.3%` in Tree wall time and `1.9%` overall;
  - this was below the explicit 10% acceptance threshold and was removed completely.
- Tests on final accepted source:
  - `[Mixed]~[Integration]~[Performance]`: 67 assertions / 5 cases passed;
  - `[Tree]~[Integration]`: 4 assertions / 1 case passed;
  - full `[Mixed]` run was stopped at the explicit 30-minute ceiling while still CPU-active inside its multi-style integration loop; it had emitted legacy `Potentially lost branch` diagnostics but no Catch2 assertion failure before the stop. Do not report this as a passing full suite.
- Build artifact used for validation:
  - `build-vulkan/tree-perf/instrumented-current/runtime/magpie-slicer.exe` and `MagpieSlicer.dll`;
  - the normal `build-vulkan/src/Release/MagpieSlicer.dll` remained locked by stale CLI PID 16064, so final DLL linking used an isolated output directory;
  - user-visible GUI PID 10308 remained open and responsive throughout.
- No commit, push, package, release, or upload was performed.

# 20. 2026-08-28 Documentation Consolidation

- Backed up 37 project-authored Markdown files before editing to `.codex-backups/md-feature-review-20260828-064433`, preserving relative paths.
- Added `docs/MAGPIE_FEATURE_CHANGES_2.5.0.0.7.md` as the current feature-by-feature inventory. It separates published-release behavior, current unreleased worktree changes, current verification and historical records.
- Reviewed the existing Markdown against current source, release notes, test names and the live git diff.
- Corrected `docs/MAGPIE_VERSIONING.md` from stale 0.3 examples to the current 0.7 representation.
- Corrected the contradictory status and stale target branch at the top of `docs/MIXED_AUTO_SUPPORT_PLAN.md`.
- Marked `docs/MAGPIE_CODE_AUDIT.md` as a historical commit snapshot rather than a current finding list.
- Marked `release-notes-2.5.0.0.7.md` as the historical published installer state because the current unreleased worktree removes Tsunami.
- Linked the current feature inventory from the Korean and English READMEs.
- `docs/magpie-vulkan-slicer.md` was reviewed and already matches the current Tree/Mixed whole-slice CPU policy, so it was not changed.
- `docs/PROJECT_CONTRACTS.md` remains normative rather than historical and was not changed.
- No production source, build, test, package, commit or push was performed for this documentation task.

# 21. 2026-08-28 Automatic Support Defect Correction Pass

- Before production edits, preserved the complete dirty working state under `.codex-backups/pre-fix-20260828-151000`. `tracked-working.patch` SHA-256 is `DEE7A2897C52100008A1E306154022AAF4770AB36B1DEEAD51B3E06B42A15F1F`; the backup also records HEAD, 64 copied changed/untracked files and eight tracked deletions.
- Fixed Default Resin max-bridge-length zero runaway in SLA neighborhood/routing helpers. The final isolated Default/Branching/common mutation sweep passes 34/34.
- Fixed cancellation propagation through Cura custom stages and Classic Tree collision/avoidance/node/toolpath stages. `[Cancellation]` passes six assertions.
- Preserved Cura automatic and painted-enforcer demand as independent channels; `[CuraPaint]` passes.
- Fixed Prusa normal wall count zero, legacy Tree wall-key migration, selected Tree body-flow/nozzle ownership, auto-width Tree geometry, and runtime Korean MO freshness.
- Root-caused Cura/Mixed slowness against current CuraEngine source `f53065705888f6264d728c9e68b356a036072785`: Magpie's port omitted per-layer `Simplify` after area joining. Adding process-resolution simplification reduced the exact Cura Bunny from about 142.82 s to 37.12 s and propagation from about 100.63 s to 3.75 s.
- The same semantic-owner fix reduced the exact Mixed Cura+Hybrid Bunny from 151.75 s to about 17.22 s and Tree collision from 36.47 s to about 1.51 s while retaining 15,495 collision jobs. No rejected Vulkan AABB cache, parallel node mutation, topology-dependent worker commit or branch-diameter enlargement was restored.
- Corrected Organic's unconditional critical diagnostic: a returned recovery result is logged as recovered without branch loss; critical is reserved for actual recovery failure. The 0.4 mm/60-degree Stanford Bunny Mixed worst-case passes 11 assertions with only the recovery warning.
- Final Release build succeeded for `OrcaSlicer` and `fff_print_tests`. Full CTest completes 393/393 entries with zero failures in 24.85 s; the single environment-filtered Resin entry is intentionally skipped and covered by the separate 34/34 run.
- Actual-slice support-feature matrix passes 28/28. The verifier now retains the active `;WIDTH` across role transitions, avoiding four false failures where interface extrusion legitimately reused the previous width without another marker.
- Windows PowerShell 5.1 verification fixes also cover null redirected `ExitCode`, UTF-8 Korean strings without BOM, and Catch2 compact output containing an intentional skip.
- Final Core release-readiness rerun passes 12/12. Evidence is `artifacts/code-audit-20260828/release-readiness-core-final-rerun/summary.json`.
- Live coordinate-based GUI automation was not run because the host contract prohibits it. Native schema/enum/visibility mapping, runtime MO, CLI G-code and process-liveness paths were verified instead.
- No commit, package, release, upload or push was performed.

# 22. 2026-08-28 Version 2.5.0.0.8 Local Installer Candidate

- Advanced the single version owner in `version.inc` from revision 7 to 8. The public/display/installer version is `2.5.0.0.8`, the internal SemVer-compatible value is `2.5.0-modified.0.8`, and the Windows numeric version is `2.5.0.8`.
- Removed only the unconditional upstream Orca application-version check from `GUI_App::post_init()`. Profile/plugin synchronization remains in startup, and the explicit Help-menu update check remains available through `check_new_version_sf(true, 1)`.
- Reconfigured and built the Release GUI and NSIS installer successfully with Vulkan slicing enabled and ordinary Magpie branding.
- Local installer: `build-vulkan/installer/20260828-174933/MagpieSlicer_Windows_Installer_V2.5.0.0.8_x64.exe`.
- Installer size: `142092554` bytes. SHA-256: `26DC29254C43ACBC0BD59067BD140A449D3FC68F03B9E447E68BBDD7D0F083AF`.
- NSIS archive integrity passed for 15,250 entries. Extracted `magpie-slicer.exe` and `MagpieSlicer.dll` hashes exactly match the Release build outputs; the packaged executable reports FileVersion and ProductVersion `2.5.0.0.8`.
- No installer execution, elevation prompt, live printer action, commit, push, upload or GitHub release was performed.

# 23. 2026-08-31 Documentation Review and Existing Build Backup

- Reconciled the routing header, feature inventory, Mixed plan, audit links, local release notes, and the separate `magpie-md` index with the existing 2.5.0.0.8 candidate. Historical sections and rejected approaches were retained.
- Added `docs/MAGPIE_STATE_AND_BACKUP_2026-08-31.md` as the current entry point for feature status, evidence boundaries, checksums, and restore scope.
- Found and corrected a documentation-level verification overclaim: the final readiness support-feature 28/28 run uses default `tests/data/overhang.obj`, not full Stanford Bunny. Its G-code thresholds are 60 degrees in 19 cases, 70 in three, and 90 in six. The earlier Bunny/60-degree 28/28 run preceded production fixes and the strengthened width assertions. The final-candidate full Bunny matrix is therefore still unverified.
- Existing final CTest, Resin and Core logs/results remain 393 entries/zero failures/one intentional skip, 34/34 and 12/12 respectively. These are retained results from before the version/startup-check packaging change, not newly run tests.
- Copied 16,394 files (2,599,951,206 bytes) to `C:\Users\svadmin\Documents\Codex\MagpieBackups\2.5.0.0.8-20260831-094611`, checking source/copy SHA-256 per file. This includes the installer, all Release root files including symbols, 15,192 materialized resource files, build metadata, pre-edit Markdown, dirty-source snapshots/patch, and evidence logs/results.
- The backup is a local preservation copy, not a new portable release or a complete independent source/toolchain mirror. Originals were neither deleted nor moved.
- Rechecked the backup NSIS archive successfully. Extracted its EXE/DLL for comparison with the preserved Release binaries; see the machine-readable verification record under `artifacts/document-build-backup-20260831`.
- No production source edits, rebuild, new slicing test, GUI automation, installer execution, commit, push, upload, release, or physical-printer action were performed in this documentation/backup task.
