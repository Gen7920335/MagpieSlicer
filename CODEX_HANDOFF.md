...
- **Next step — Micro Branch proper, which is also what removes the temporary fan-gap acceptance.** With micro forced on, hollow gear now plans without failing but fails `test_support_material.cpp:866`, an assertion requiring the root and contact layers to hold equal entity counts; micro trees add contact-layer entities, so that fixture has to be re-specified for micro mode before micro can become the shipping configuration.
