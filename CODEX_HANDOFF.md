...
- **Run `bash scripts/tsunami_session_start.sh` first.** It prints the reading order with every document path, checks each one exists and is tracked, reports branch, head, dirty files and whether any temporary instrumentation is still in the tree, lists the permanent diagnostics with the grep that reads them, gives the suite status with per-case timings, and names the next steps. `--verify` also builds and times the five main cases.
- **Start here for Tsunami: `docs/TSUNAMI_SESSION_2026-08-18.md`.** It is self-contained: current suite status with per-case timings, the build and run commands, every permanent diagnostic and what it says, what changed with the measurement behind it, all five failures with their causes named, the nine refuted claims not to re-derive, and the ordered next steps. Read it before proposing any cause in root selection, coverage, or micro planning.
...
- **Root selection now reports itself, and multi-column's undersized roots are a reach limit, not a selection bug.** `Tsunami root selection:` prints on any selection whose winner does not span its target: direct candidates, contour regions, regions dropped as too far, contour candidates, and the winner's ribs, coverage, bed area and score. Measured — multi-column `regions=4 too_far=3 candidates=18 ribs=2 coverage=0.143 score=142.7`, hollow gear `regions=1 too_far=0 candidates=12 ribs=31 coverage=1.000 score=999.85`. **Three of four contour regions sit beyond `maximum_xy_distance` of 13.3 mm**, so one region supplies every candidate and its best spans a seventh of the target. The score is arithmetically correct at 1000 x coverage; there is nothing better to pick. The hollow gear never showed this because its one region covers the target outright, so **its health was luck of geometry rather than evidence the stage works**.
- **This makes the refuted "multiple roots per target" item conditional rather than dead.** It was refuted on the hollow gear, where reachability was measured not to be the binding constraint. On multi-column it **is** the binding constraint. Do not carry either verdict across models; the same question has different answers per geometry, and the root selection line now says which case a model is in.
- **RETRACTED within the hour: multi-column does not fail 2 assertions, it fails 144 of 320.** The 2 was read with the bad `REQUIRE` still aborting the case. With the abort gone the per-overhang checks all run, and most of them fail — the fixture's real state is that support does not reach the overhangs, which matches Tsunami falling back on those layers.
- **Refuted: the direct-projection early return is not why roots come out tiny.** `select_root_candidate` returns the moment `make_direct_root_candidates` yields anything, skipping the contour search entirely, and `allow_direct_projection` is on whenever micro is off — which looked like the whole story. Forcing it off leaves `root_ribs` at 2, 8 and 6, byte for byte. **Root sizing is decided somewhere else and there is no diagnostic anywhere in root selection**, which is why three separate failures all dead-end here. The next step is to add one: candidates generated, rejections by reason, and the winner's ribs, coverage and score. Do not guess a mechanism before that exists; this was the ninth wrong cause in one session and every one of them was named before the observable was in place.
- **`covers every overhang of a multi-column model` is diagnosed, and the defect is root sizing, not the fixture.** Two fixture assertions were wrong and one of them, a `REQUIRE` on `support_layers[layer_index - 1]`, aborted the case before anything downstream ran — the same Z-gap index bug already fixed on the hollow gear. Both are corrected (serving layer is now found by `print_z`), and what they were hiding is that **Tsunami falls back here**. Measured: for a 51.19 mm2 target the root comes out at **2 ribs and 2.41 mm2 of bed contact**, against 29 ribs and 37.84 mm2 on the hollow gear; one branch is planned, it covers 12.3 mm2, and planning fails with 38.87 mm2 uncovered. For the 71.84 mm2 target the first root gets 8 ribs, plans nothing (`no_plan_reasons=[collision=4]`, `support_empty=4` — the columns block it), retries at 6 ribs, and ends 54.92 mm2 short. **Root selection is not sizing the trunk to the target**, which is the same area §7.0.1 rewrites and is upstream of everything else on this fixture. The remaining `support_islands.size() >= target_regions.size()` check (2 against 4) is a count proxy of the kind removed elsewhere, but it is deliberately left in place: while planning falls back that count describes Normal's output, so deleting it would hide the sizing defect rather than a bad assertion.
