...
- **Next step — the last 0.78 mm2 on snug micro.** Everything cheap has been spent: the residue is no longer explained by a shared constant, a starved ring, or a unitless threshold. Locate it first with the `Tsunami micro reach rejection` line's largest-piece fields, exactly as on the hollow gear, and only then decide between a second trunk and a contract change. Do not reason from `uncovered_mm2` across micro on and off.
