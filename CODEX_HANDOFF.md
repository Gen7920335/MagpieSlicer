...
- **Next step — the fixture, then the gain-versus-outcome discrepancy.** Micro cannot become the shipping configuration while `test_support_material.cpp:866` requires the root and contact layers to hold equal entity counts, which micro trees cannot satisfy; that assertion needs re-specifying for micro mode. Separately, at two branches per turn with micro **off**, hollow gear target 1 gains two branches and its uncovered area does not move at all — a branch is accepted on positive estimated gain and then changes the outcome by nothing, which is the same estimate-versus-outcome shape as every entry in the defect table. snug overhang's 1.84 mm2 is unaffected by any of this and stays collision-limited under parallelism.
