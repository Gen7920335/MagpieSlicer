...
- **Next step — split the three jobs `maximum_bridge_distance` does.** It is model clearance before the demand cut, footprint dilation when crediting coverage, and contact sampling spacing, all one number, and widening it under micro changes the *problem* rather than the solution. Until they are separate, no cross-mode coverage number can be trusted, which is exactly how the "micro solves snug" reading happened. Only after that is it worth asking whether the remaining snug residue needs a second trunk.
